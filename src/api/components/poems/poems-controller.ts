...
import * as service from './poems-service';
// Tipos
...
async function get_all_poems_of_author(req: Request, res: Response) {
    const { page, perpage } = req.query;
    const id = req.params.id;
    ...
    service.get_all_poems_of_author(current_page, current_perpage, id)
        .then((ok: Response_data) => res.status(ok.status).json(ok))
        .catch((err: Response_data) => res.status(err.status).json(err))
}
